...
ReplyParser.prototype._parseResult = function (type) {
  ...
  if (type === 43 || type === 45) { // + or -
    ...
    return parseInt(smallToString(this._buffer, start, end), 10);
  } else if (type === 36) { // $
...
ReplyParser.prototype._packetEndOffset = function () {
  ...
  if (offset >= bufferLength) {
    throw new IncompleteReadBuffer('didn\'t see LF after NL reading multi bulk count (' + offset + ' => ' + bufferLength + ', ' + this._offset + ')');
  }
